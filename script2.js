...
function getRestfulParmas() {
    let roomId = $('#roomlist').val() || defaultRoomId,
        ...
        rtmpUrl = $('#rtmpurl').val() || $('#dashurl').val(),
...
function restCreateSipcalls() {
    ...
    let audio = false, video = false, parameters = undefined;
    if (audioFrom) {
        audio = true;
    }
    if (videoFrom) {
    ...
    if (bitrate || resolution || frameRate || kfi) {
        parameters = {
            resolution: resolutionName2Value[resolution],
            framerate: frameRate,
            bitrate: bitrate,
...
/*app.post('/rooms/:room/analytics', function(req, res) {
  'use strict';
   var room = req.params.room,
       ...
       icsREST.API.startAnalytics(room,
                  algorithm,
                  media,
                  function(info) {
                    res.send(info);
...
app.delete('/rooms/:room/analytics/:id', function(req, res) {
  ...
  var room = req.params.room,
      id = req.params.id;
      icsREST.API.stopAnalytics(room,
                id,
                function(result) {
                  res.send(result);
                },
...
function publicationEvent(publication) {
    ...
    errorListener = () => {
        console.log(" error occurred on the publication");
    };
    ...
    publication.addEventListener('error', errorListener);
}
...
function subscriptionEvent(subscription) {
    ...
    errorListener = () => {
        console.log(" error occurred on the subscription");
    };
...
//LocalStream action
function _stopLocal(localStream, name) {
    if (localStream) {
        localStream.mediaStream.getTracks().forEach(track => {
            track.stop();
        })
...
function stopLocal() {
    ...
    _stopLocal(localStream, name);
}

function _createLocalStream(videoSource, audioSource, resolution, extensionId, audioDeviceId, videoDeviceId, frameRate) {
    let audio, video;
    if (!audioSource) {
        audio = false;
    ...
    return new Promise(function (resolve, reject) {
        MediaStreamFactory.createMediaStream(mediaStreamDeviceConstraints)
            .then((mediaStream) => {
                ...
                let localStream = new LocalStream(mediaStream, streamSourceInfo, { testAttributes: 'test' });
...
function createLocalStream() {
    ...
    let frameRate = parseInt($('#framerate').val()) || 30;
    ...
    _createLocalStream(videoSource, audioSource, resolution, extensionId, audioDeviceId, videoDeviceId, frameRate);
...
function publish() {
    ...
    let videoCodecpub = $('#videoCodecpub').val();
    let simulcastAudio = $('#simulcastAudio').val();
    let {
        audioCodec,
        videoCodec
...
$('#availableremotestream').change(function () {
    ...
    let issimulcast = getParameterByName('simulcast') || false;
    let audio
    let video
    ...
    if(videosetting.length === 0 && videoextracapa === undefined){
        ...
        if (issimulcast && !(remoteStream.id.includes('common')) || videorid != undefined){
            ...
            if(video){
                ...
                let frameRates = new Array();
                let keyFrameIntervals = new Array();
                let rids = new Array();
                video.forEach((item) => {
                    let {
                    ...
                    supportedVideoCodecs.push(supportedVideoCodec);
                    bitrateMultipliers.push(bitrateMultiplier);
                ...
                rids && addOption($('#supportedRid'), rids);
            }
        }
...
function getSubOptions() {
    ...
    let rid = $('#supportedRid').val() || undefined;
    return {
        remoteStreamId,
        trackKind,
        videoCodec,
...
function _subscribe(remoteStream, trackKind, videoCodec, audioCodec, resolution, frameRate, bitrate, kfi, rid) {
    let audio, video;
    if (trackKind === 'audio') {
        video = false;
    ...
    return new Promise(function (resolve, reject) {
        client.subscribe(remoteStream, subOptions)
            .then((subscription) => {
                subscription.originId = remoteStream.id;
                subscriptions.set(subscription.id, subscription);
                displayStream(remoteStreamMap.get(remoteStream.id));
...
function sendMessage1() {
    ...
    let inter = setInterval(() => {
        createToken(defaultRoomId, 'testroom', 'presenter', (token) => {
...
window.onload = function () {
    ...
    var simulcastVcodec = getParameterByName('simulcastVcodec') || 'VP8';
    let publishOptions;
